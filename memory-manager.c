...
PTEntry** get_all_dependents(int tid, int page_num) {
	...
	int i;
	int first_page_num = orig_page_num - left_pages;
	for(i=0; i<total_pages; i++){
		dependents[i] = book_keeper[first_page_num + i].entry;
	}
...
PTEntry** protectmemory(int tid, int* addr){
    ...
    printf(ANSI_COLOR_YELLOW"PROTECTMEMORY: PROT_WRITE and swapping page if not resident!\n" ANSI_COLOR_RESET);

	int page_num = (addr - memory[0])/(PAGE_SIZE);
	swap(tid, page_num);

	printf(ANSI_COLOR_YELLOW"PROTECTMEMORY: Getting all dependents of page!\n" ANSI_COLOR_RESET);
